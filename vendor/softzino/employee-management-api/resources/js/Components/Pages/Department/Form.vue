  ...
  form.submit({
    onSuccess: () => {
      if (page.props.flash?.success) {
        showToast('success', page.props.flash.success, 3000, 'top-right');
      }
      closeForm();
    },
    onError: () => {
    ...
    >
      <template #content>
        <div class="form-container px-1 text-left">
          <h3 class="title">
            {{ props?.department ? 'Edit Department' : 'Create Department' }}
          </h3>
          <p class="sub-title">
            You are about to
            {{ props?.department ? 'edit' : 'create' }} a department.
          </p>
          ...
          <form @submit.prevent="handleFormSubmit" class="mt-5">
            ...
            <div class="mt-5 flex justify-end gap-3">
              <Button class="base-button" type="button" @click="closeForm">
                Close
              </Button>
              <Button
